...
void teleport_player(int dis, bool safe)
{
	...
	if (!safe)
	{
		...
		if ((cave_feat[y][x] == FEAT_TREE) && (randint0(2) == 0))
		{
...
int resist_damage(int dam, byte resist, byte rand_factor)
{
	...
	if ((rand_factor != 0) & (resist_percentage > 0) & (resist_percentage < 100))
	{
		...
		/* Randomize */
		resist_percentage = resist_percentage - range + randint0((2 * range) + 1);
	}
...
bool pois_hit(int pois_inc)
{
	...
	if (!p_resist_strong(P_RES_POIS))
	{
		...
		if (p_ptr->poisoned)
		{
			...
			if (set_poisoned(p_ptr->poisoned + 2 + randint1((pois_inc + 2) / 3) +
				(pois_inc / 3)))
				did_harm = TRUE;
		}
		else
		{
			...
			if (set_poisoned(p_ptr->poisoned + 8 + randint1((pois_inc + 1) / 2) +
				(pois_inc / 2)))
				did_harm = TRUE;
		}
	}
...
static bool project_m(int who, int y, int x, int dam, int typ, int flg)
{
	...
	switch (typ)
	{
		...
		case GF_CHAOS:
		{
			...
			if (seen) obvious = TRUE;
			do_poly = TRUE;
			do_conf = randint1(dam > 240 ? 20 : dam / 12);
		...
		{
			/* Attempt a saving throw. */
			if ((randint1(3 * r_ptr->level / 2) > p_ptr->lev * 2) ||
				randint0(4) == 0) note = " fights off your spell.";
			else
			{
		...
		case GF_HOLD_UNDEAD:
		{
			...
			{
				/* Attempt a saving throw. */
				if ((randint1(r_ptr->level) > p_ptr->lev * 2) ||
					randint0(5) == 0) note = " fights off your spell.";
				else
				{
...
static bool project_p(int who, int d, int y, int x, int dam, int typ)
{
	...
	switch (cave_feat[y][x])
	{
		...
		case FEAT_TREE:
		{
			if ((randint1(8) == 1) && (check_ability(SP_WOODSMAN)))
			{
	...
	switch (typ)
	{
		...
		case GF_ROCK:
		{
			...
			/* Try for Evasion */
			if (check_ability(SP_EVASION) & (randint1(75) <= p_ptr->evasion_chance))
			{
				/* Message */
		...
		case GF_HELLFIRE:
		{
			...
			if ((!self) && (!check_save(5 * r_ptr->level / 4)))
			{
				...
				if (!p_ptr->no_fear)
				{
					(void)set_afraid(p_ptr->afraid + randint1(30) + r_ptr->level * 2);				}
		...
		case GF_SOUND:
		{
			...
			if ((dam > randint0(30 + dam / 2)) && !p_resist_pos(P_RES_SOUND))
			{
		...
		case GF_WATER:
		{
			...
			if (fuzzy) msg_print("You are hit by something!");
			if ((!p_resist_pos(P_RES_SOUND)) && (randint0(2) == 0))
			{
				(void)set_stun(p_ptr->stun + randint1(5 + dam / 10));
			}
			if ((!p_resist_pos(P_RES_CONFU)) && (randint0(2) == 0))
			{
		...
		case GF_STORM:
		{
			...
			if ((randint0(2) == 0) && (!p_resist_pos(P_RES_CONFU)))
			{
...
static bool project_t(int who, int y, int x, int dam, int typ, int flg)
{
	...
	switch (typ)
	{
		...
		case GF_PLASMA:
		{
			...
			if ((cave_feat[y][x] == FEAT_TREE) && (dam > randint1(400) + 100))
			{
				/* Forget the tree */
				cave_info[y][x] &= ~(CAVE_MARK);
